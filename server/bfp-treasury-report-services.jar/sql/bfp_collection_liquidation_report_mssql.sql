...
from ( 
	...
	from ( 
		select 
			...
			convert(date, cv.controldate) as liq_controldate, 
		...
		where cv.controldate >= $P{startdate} 
			and cv.controldate < $P{enddate} 
		group by 
			cv.objid, cv.controlno, convert(date, cv.controldate), ci.receiptid, ci.acctid 
		union all 
		select 
			cv.objid as liq_objid, cv.controlno as liq_controlno, 
			convert(date, cv.controldate) as liq_controldate, 
		...
		where cv.controldate >= $P{startdate} 
			and cv.controldate < $P{enddate} 
		group by 
			cv.objid, cv.controlno, convert(date, cv.controldate), ci.receiptid, ci.refacctid 
		union all 
		select 
			cv.objid as liq_objid, cv.controlno as liq_controlno, 
			convert(date, cv.controldate) as liq_controldate, 
	...
	inner join itemaccount ia on ia.objid = tmp2.acctid 
where c.voided = 0 
order by tmp2.liq_controldate, tmp2.liq_controlno, convert(date, c.receiptdate), c.series 
